...
def login():
    try:
        ...
        if not user:
            return make_response(
                {'message': 'Please create an account', 'status': 401}, 401
            )
...
def getAllFunds(current_user):
    try:
        ...
        totalSum = 0
        if funds:
            totalSum = Funds.query.with_entities(db.func.round(
                func.sum(Funds.amount), 2)).filter_by(userId=current_user.id).all()[0][0]
        return make_response({
...
def updateFund(current_user, id):
    try:
        ...
        if funds == None:
            return make_response({"message": "Unable to update", 'status': 409}, 409)
        data = request.json
        amount = data.get('amount')
        try:
            if amount:
                funds.amount = amount
                funds.updated_at = datetime.utcnow() + timedelta(minutes=30)

            db.session.commit()
            return make_response({"message": funds.putserialize}, 200)
        except Exception as e:
            return make_response({
                'message': 'Invalid input'
            })
        ...
        print(e)
        return make_response({"message": "Unable to process", 'status': 409}, 409)
...
def deleteFund(current_user, id):
    try:
        ...
        if fund == None:
            return make_response({"message": f"Fund with {id} not found", 'status': 404}, 404)
        db.session.delete(fund)
        db.session.commit()
        return make_response({'message': 'Deleted', 'status': 200}, 200)
